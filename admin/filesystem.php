...
function write_robots() {
    ...
    if(is_file(BASE_DIR.'robots.txt')) {
        ...
        $lines = array('User-agent: *','Disallow: /'.ADMIN_DIR_NAME.'/','Disallow: /'.CMS_DIR_NAME.'/','Disallow: /tmp/','Disallow: /docu/');
    }
    ...
    $text = implode("\n",$lines)."\n";
    if($CMS_CONF->get('usesitemap') == "true") {
        $text = 'Sitemap: '.HTTP.$_SERVER['SERVER_NAME'].''.URL_BASE.'sitemap.xml'."\n".$text;
    }
