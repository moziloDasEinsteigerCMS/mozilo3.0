...
var delete_handler = function (event) {
    ...
    var li_cat_page = find_li_cat_page(that),
        this_table = li_cat_page.find("div").eq(0),
        dialog_text = "<div id=\"dialog-del\"><div><b>" + this_table.find(".js-normal-in-name").text() + "<\/b>";
    if(!is_page(this_table.find(".js-in-cat-page").val()) && !get_target(this_table.find(".js-in-cat-page").val())) {
        if(li_cat_page.find(".js-li-page").length > 0) {
            dialog_text += "<b>" + mozilo_lang["pages"] + ":<\/b><div>";
            li_cat_page.find(".js-li-page").each(function(index) {
                dialog_text += "<div>" + $(this).find(".js-page-name").text() + "<\/div>";
            });
            dialog_text += "<\/div>";
        }
        if(this_table.find(".js-cat-files").val() != "false") {
            dialog_text += "<b>" + mozilo_lang["files"] + ":<\/b><div>";
            var files = this_table.find(".js-cat-files").val().split("-#-");
            for (var i = 0; i < files.length; ++i)
                dialog_text += "<div>" + files[i] + "<\/div>";
            dialog_text += "<\/div>";
        }
    }
    dialog_text += "<\/div><\/div>";
...
var in_name_enter_handler = function(event) {
    ...
    if(event.which == 13) { // enter
        var li_cat_page = find_li_cat_page(that);
        if(li_cat_page) {
            li_cat_page.find("div").eq(0).find(".js-edit-rename").trigger("click");
        }
        return false;
    } else if(event.which == 27) { // esc
        var li_cat_page = find_li_cat_page(that);
        if(li_cat_page) {
            li_cat_page.find("div").eq(0).find(".js-edit-rename").data("rename-mode","false");
            change_to_normal_mode(li_cat_page.find("div").eq(0));
        }
        return false;
    }
};
...
var rename_handler = function (event) {
    ...
    if(!li_cat_page)
        return false;
    var this_table = li_cat_page.find("div").eq(0);
...
var option_page_sortable = {
    ...
    placeholder: "mo-in-ul-li mo-tag-height-from-icon ui-corner-all ui-state-highlight mo-placeholder mb",
...
//$('#out').html($('#out').html()+"drop<br>");
            ...
            if(!get_target(ui.draggable.find(".js-in-cat").attr("name"))) {
                ...
                var new_ul = $("<div class=\"js-ul-pages  mo-in-ul-ul mo-padding-bottom\"><\/div>")
        ...
        placeholder: "mo-placeholder mo-li mo-li-head-tag mo-li-head-tag-no-ul mo-tag-height-from-icon ui-corner-all ui-state-highlight mb",
        start: function(event, ui) {
