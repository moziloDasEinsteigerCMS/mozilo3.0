...
function get_next_free_name(in_cat_page,name) {
    // name ist leer also gleich raus hier sowas gibts nicht :-)
    if(name.length < 1) {
        return false;
    }
    // wenns ein name ist mit copy syntax
...
function change_status_pages(this_li) {
    var item_li_cat = this_li.parents(".js-li-cat"),
        item_status = item_li_cat.find("div").eq(0).find(".js-status");
...
function make_input_target(cat_page) {
    ...
    if(!curent_target)
        return "";
    var inputs = "<span class=\"mo-bold mo-padding-right\">" + mozilo_lang["target"] + "<\/span>" + " <form>";
    var for_id = new Date();
    for_id = for_id.getTime();
    for(var i = 0; i < target_array.length; i++) {
        inputs += "<input id=\"target" + for_id + i + "\" name=\"radio\" type=\"radio\" value=\""+target_array[i]+"\" class=\"js-in-radio js-make-input mr\"";
        if(target_array[i] == curent_target) {
            inputs += " checked=\"checked\"";
        }
        inputs += " \/>";
        inputs += "<label for=\"target" + for_id + i + "\" class=\"mo-padding-right\">" + mozilo_lang[target_array[i].replace(/-|_/g,"")] + "<\/label>";
    }
    return inputs + "<\/form>";
}
...
function make_input_ext(cat_page) {
    ...
    if(!curent_ext || curent_ext == EXT_LINK)
        return "";
    var inputs = "<span class=\"mo-bold mo-padding-right\">" + mozilo_lang["page_status"] + ":<\/span>" + "<form>",
        for_id = new Date();
    for_id = for_id.getTime();
    for(var i = 0; i < ext_array.length - 1; i++) {
        inputs += "<input id=\"status" + for_id + i + "\" name=\"radio\" type=\"radio\" value=\""+ext_array[i]+"\" class=\"js-in-radio js-make-input mr\"";
        if(ext_array[i] == curent_ext) {
            inputs += " checked=\"checked\"";
        }
        inputs += " \/>";
        inputs += "<label for=\"status" + for_id + i + "\" class=\"mo-padding-right\">" + mozilo_lang[ext_array[i]] + "<\/label>";
    }
    return inputs + "<\/form>";
}
