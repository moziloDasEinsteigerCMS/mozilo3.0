...
function send_editor_preview() {
    ...
    $.ajax({
        ...
        success: function(getdata, textStatus, jqXHR){
            ...
            if(getdata == "true") {
                var prev_iframe = $('<iframe style="overflow:visible; border: none; text-align:left; width:100%; height:100%;"><\/iframe>').prop('src', prev_url);
...
//     }
    ...
    if(action_activ == "catpage") {
        ...
        $("#prev-pageedit-box").dialog({
        		title: mozilo_lang["button_preview"],
        		buttons: [{
                text: mozilo_lang["close"],
                click: function() {
                    $(this).dialog("close");
