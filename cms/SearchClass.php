...
class SearchClass {
    ...
    function getFindPagesMenuList($matchingpages = array(),$searchquery = false,$css = "searchmap") {
        global $CatPage, $SEARCH_REQUEST, $language;

        $searchresults = '<h2 class="heading2">' . $language->getLanguageValue("message_searchresult_0",$SEARCH_REQUEST) . '</h2>';
        ...
        if($this->hidecatnamedpages == "true")
		  ...
		  $searchresults ='<div id="searchresult">';        
        $searchresults .= '<h2 class="heading2">'.$language->getLanguageValue("message_searchresult_1",trim(rawurldecode($SEARCH_REQUEST))).'</h2>';        
 #       $searchresults .= '<ul class="'.$css.'">';
        foreach($matchingpages as $cat => $tmp) {
            $searchresults .= '<div class="'.$css.' result-list">';
            $catname = $CatPage->get_HrefText($cat,false);
            if($hidepage and isset($matchingpages[$cat][$cat])) {
                $searchresults .= '<span class="'.$css.' result-list-header">'.$this->makeSearchMenuLink($cat,$cat,$searchquery).'</span>';
                unset($matchingpages[$cat][$cat]);
            } else
                $searchresults .= '<span class="'.$css.' result-list-header">'.$catname.'</span>';
            if(count($matchingpages[$cat]) > 0)
 #               $searchresults .= "<ul>";
            foreach($matchingpages[$cat] as $page => $tmp) {
                $pagename = $CatPage->get_HrefText($cat,$page);
                $searchresults .= '<div class="'.$css.' result-list-item">'.$this->makeSearchMenuLink($cat,$page,$searchquery).'</div>';
            }
 //           if(count($matchingpages[$cat]) > 0)
 #               $searchresults .= "</ul>";
            $searchresults .= "</div>";
    ...
    function highlightSearchString($content) {
        ...
        foreach($this->phrasearray as $phrase) {
            ...
            while(preg_match('/(>|^)[^<]*(?<!\<mark class\="highlight"\>)'.$phrase.'(?!\<\/mark\>)[^>]*(<|$)/isu', $content))
                $content = preg_replace('/(^[^<]*|>[^<]*)(?<!\<mark class\="highlight"\>)('.$phrase.')(?!\<\/mark\>)([^>]*<|[^>]*$)/isu', '${1}<mark class="highlight">${2}</mark>${3}', $content);
    ...
    function getSearchForm() {
        ...
        $query = str_replace(array('"',"'","[","]","{","}"),array("&quot;","&apos;","&#091;","&#093;","&#123;","&#125;"),trim(rawurldecode($SEARCH_REQUEST)));
        return '<form class="searchform" accept-charset="'.CHARSET.'" method="get" action="'.$CatPage->get_Href(false,false).'" role="search">'
                .$draft
                .'<input type="hidden" name="action" value="search">'
                .'<input type="text" name="search" value="'.$query.'" class="searchtextfield" aria-label="Search">'
 #               .'<input type="image" src="'.$LAYOUT_DIR_URL.'/grafiken/searchicon.gif" alt="'.$language->getLanguageHtml("message_search_0").'" class="searchbutton" />'
                .'<button id="searchbutton" type="submit" class="searchbutton" aria-label="Searchbutton"></button>'
                .'</form>';
    }
