 ...
 *
 * $Revision: 926 $
 * $LastChangedDate: 2024-01-16 10:00:00 +0200 (Di, 16. Januar 2024) $
 * $Author: mozilo $
 *
...
// ------------------------------------------------------------------------------
    function getTitleAttribute($value) {
        ...
        if ($CMS_CONF->get("showsyntaxtooltips") == "true") {
            return ' title="'.str_replace('"',"&quot;",$value).'"';
          #  return ' title="'.$value.'"';
        }
        return "";
    }
...
// ------------------------------------------------------------------------------
        ...
        if(count($GLOBALS['syntax_anchor_ueber']) < 1)
            return NULL;
        global $language;
        $tableofcontents = '<div id="tableofcontents">';
        $tableofcontents .= '<h2 class="heading2">'.$language->getLanguageValue("message_tableofcontents_0").'</h2>';
        $tableofcontents .= '<div class="tableofcontents result-list">';
        foreach($GLOBALS['syntax_anchor_ueber'] as $value => $count_type) {
            ...
            if ($count_type['type'] >= "2") {
                $tableofcontents .= '<div class="tableofcontents result-list-item blind"><div>';
            }
            if ($count_type['type'] >= "3") {
                $tableofcontents .= '<div class="tableofcontents result-list-item blind"><div>';
            }
            $tableofcontents .= '<div class="tableofcontents result-list-item toc_'.$count_type['type'].'">'.$link.'</div>';
            if ($count_type['type'] >= "2") {
