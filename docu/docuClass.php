...
#        if($this->docu_writer)
            $this->docu_artikel .= '<script src="'.BASE_URL_DOCU.'jquery/docu.js"></script>'."\n";
            
//            .'<script src="'.BASE_URL_DOCU.'jquery/toggle_jquery.js"></script>';
...
#        if($this->isplugin or ($this->dialog and !is_array($this->menu[$this->artikel])))
 ...
 //       $submenu = '<div class="ui-dialog-titlebar mo-menu-tabs ui-tabs-nav">';

        $cssaktiv = "do-tab-label mo-ui-state-hover";
        if(strlen($this->subartikel) < 1)
            $cssaktiv = "do-tab-label ui-tabs-selected ui-state-active active ";
        $submenu = '<div class="ui-state-default '.$cssaktiv.'">'.$this->makeDocuLink('<b>'.$this->getDocuLanguage("domenu_".$this->artikel).'</b>',$this->artikel).'</div>';
        ...
        if(isset($this->menu[$this->artikel]) and is_array($this->menu[$this->artikel])) {
            foreach($this->menu[$this->artikel] as $subnav => $tmp) {
                ...
                if($this->subartikel == $subnav)
                    $cssaktiv = " do-tab-label ui-tabs-selected ui-state-active active";
                $submenu .= '<div class="'.$cssaktiv.'">'.$this->makeDocuLink('<b>'.$this->getDocuLanguage("domenu_".$this->artikel."_".$subnav).'</b>',$this->artikel,$subnav).'</div>';
            }
        }
//        $submenu .= '</div>';
        return $submenu;
    ...
    function makeDocuMenu() {
        $docmenu = '<div class="admin-side">'
        .'<input class="menu-btn" type="checkbox" id="menu-btn">'
        .'<ul class="nav do-menu ui-helper-reset">';
        foreach($this->menu as $nav => $tmp) {
            ...
            if($this->artikel == $nav)
                $cssaktiv = " active";
            $docmenu .= '<li class="'.$cssaktiv.'">'.$this->makeDocuLink(''.$this->getDocuLanguage("domenu_".$nav).'',$nav).'</li>';
        }
    ...
    function makeDocuLink($text,$artikel,$subartikel = false,$language = false,$is_help_icon = false) {
        ...
        return '<a class="flex" href="'.BASE_URL_DOCU.DOCU_PHP.'?artikel='.$artikel.$sub.$this->dialog.$url_lang.'"'.$id.'>'.$text.'</a>';
    }
