...
function readTemplate($template,$pagecontent) {
    ...
    $HTML = str_replace('{CONTENT}','<div itemscope itemtype="https://schema.org/Article">---content~~~'.$pagecontent.'~~~content---</div>',$template);
...
function getSiteMap() {
    ...
    $sitemap .= '<h2 class="heading2">'.$language->getLanguageValue("message_sitemap_0").'</h2>';
    ...
    foreach ($categoriesarray as $currentcategory) {
    	$sitemap .= '<div class="sitemap result-list">';
        $sitemap .= '<span class="sitemap result-list-header">'.$CatPage->get_HrefText($currentcategory,false).'</span>';
#        $sitemap .= "<ul class=\"unorderedlist\">";
        ...
        foreach ($contentarray as $currentcontent) {
            ...
            $sitemap .= '<div class="sitemap result-list-item">'.$CatPage->create_LinkTag($url,$urltext,false,$titel).'</div>';
        }
#        $sitemap .= "</ul>";
$sitemap .= '</div>';
    }
